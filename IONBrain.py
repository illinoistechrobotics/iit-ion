...
class IONBrain(Brain):
	...
	for i in range(0, self.slength):
		#At this point we currently have the location of a blocked square in polar. Convert to Cartesian, then convert so Grid.
		self.blockmap[int(round(self.currentx + self.robot.sonar[0][self.sen[i]].value*math.cos(self.robot.th + self.sang[i])))][int(round(self.currenty + self.robot.sonar[0][self.sen[i]].value*math.sin(self.robot.th + self.sang[i])))] = 1
...
#Placeholder activity:
	self.m1 = 1
	self.m2 = 0.3
	self.robot.move(self.m1,self.m2)
